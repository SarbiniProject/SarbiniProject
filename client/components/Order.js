...
const Order = () => {
  ...
  const SendIt=(id)=>{
    axios.put("http://192.168.104.3:3000/api/sarbini/orders2/"+id,{satus2:true})
    .then(()=>{
      Alert.alert("send it")
      setTimeout(() => {
  ...
  const Tableon = async () => {
    try {
      const res = await axios.get("http://192.168.104.3:3000/api/sarbini/orderon");
  ...
  const Addnote=async(id,info)=>{
    try{
      const res = await axios.put("http://192.168.104.3:3000/api/sarbini/addnote/"+id,{note:info})
      .then(()=>{
  ...
  const DeleteProd=(id,info)=>{
    axios.put("http://192.168.104.3:3000/api/sarbini/deleteprod/"+id,{id:info})
    .then(()=>{
      console.log("deleted");
      setRef(!ref)
    })
